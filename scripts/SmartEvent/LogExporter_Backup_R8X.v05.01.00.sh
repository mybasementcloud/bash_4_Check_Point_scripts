#!/bin/bash
#
# SCRIPT Log Exporter configuration backup for R8X based systems
#
...
#
#
ScriptDate=2020-12-14
ScriptVersion=05.01.00
ScriptRevision=000
TemplateVersion=05.01.00
TemplateLevel=006
SubScriptsLevel=010
SubScriptsVersion=05.01.00
#
...
export BASHScriptFileNameRoot=LogExporter_Backup_R8X
export BASHScriptShortName=LogExporter_Backup_R8X.v${ScriptVersion}
export BASHScriptnohupName=${BASHScriptShortName}
export BASHScriptDescription="Log Exporter configuration backup for R8X based systems"
...
export BASHScripttftptargetfolder="Log_Exporter_backups"
...
if [ ${Check4SMS} -gt 0 ] && [ ${Check4MDS} -eq 0 ]; then
    ...
    echo
    echo "Continueing with Log Exporter backup..."
    echo
...
# -------------------------------------------------------------------------------------------------
# Validate we are working on an R8X system that handles the API requirements for this operation
# -------------------------------------------------------------------------------------------------
...
# -------------------------------------------------------------------------------------------------
# BackupLogExporterTarget - Backup Log Exporter Target folder
# -------------------------------------------------------------------------------------------------

# MODIFIED 2020-11-11 -\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
#

BackupLogExporterTarget () {
    #
    # repeated procedure description
    #
    ...
    expandedpath=$(cd $workfolder ; pwd)
    workfolder=${expandedpath}
    
    #expandedpath=$(cd $filepathoutput ; pwd)
    #filepathoutput=${expandedpath}
    
    #expandedpath=$(cd $filepathbackup ; pwd)
    #filepathbackup=${expandedpath}
    ...
    if [ -z $domainname ] ; then
        # No domain so either SMS or MDS level of MDSM
        
        echo 'Backup Log Exporter targets' | tee -a -i ${logfilepath}
        echo 'Work Folder :  '$workfolder | tee -a -i ${logfilepath}
        echo 'Output File :  '$filepathoutput | tee -a -i ${logfilepath}
        echo 'Backup File :  '$filepathbackup | tee -a -i ${logfilepath}
        echo '-------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
        echo | tee -a -i ${logfilepath}
        
        if [ "${sys_type_MDS}" == "true" ]; then
            mdsenv >> ${logfilepath} 2>> ${logfilepath}
        fi
        ...
        if [ -r targets ] ; then
            echo 'targets folder found, compressing to '$filepathbackup | tee -a -i $filepathoutput
            tar -cvzf $filepathbackup targets | tee -a -i $filepathoutput
            cat $filepathoutput >> ${logfilepath}
        else
            echo 'targets folder NOT found, not generating archive file '$filepathbackup | tee -a -i $filepathoutput
            cat $filepathoutput >> ${logfilepath}
        fi
        ...
        echo 'Backup Log Exporter targets' | tee -a -i ${logfilepath}
        echo 'Domain      :  '$domainname | tee -a -i ${logfilepath}
        echo 'Work Folder :  '$workfolder | tee -a -i ${logfilepath}
        echo 'Output File :  '$filepathoutput | tee -a -i ${logfilepath}
        echo 'Backup File :  '$filepathbackup | tee -a -i ${logfilepath}
        echo '-------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
        ...
        if [ "${sys_type_MDS}" == "true" ]; then
            mdsenv $domainname >> ${logfilepath} 2>> ${logfilepath}
        fi
        ...
        if [ -r targets ] ; then
            echo 'targets folder found for domain '$domainname', compressing to '$filepathbackup | tee -a -i $filepathoutput
            tar -cvzf $filepathbackup targets | tee -a -i $filepathoutput
            cat $filepathoutput >> ${logfilepath}
        else
            echo 'targets folder NOT found for domain '$domainname', not generating archive file '$filepathbackup | tee -a -i $filepathoutput
            cat $filepathoutput >> ${logfilepath}
        fi
...
#BackupLogExporterTarget $workfolder ${outputfile} $backupfile $domainname
...
# -------------------------------------------------------------------------------------------------
# GetDomainsIntoArray - collect domains into an array
# -------------------------------------------------------------------------------------------------
...
GetDomainsIntoArray () {
    ...
    #
    # "System Data" and "Global" domains are not operational for Log Exporter
    #
    #line="\"System Data\""
    #export DOMAINSARRAY+=("${line}")
...
GaiaWebSSLPortCheck
export MGMTSSLPORT=${currentapisslport}
...
export outputfileprefix='Log_Exporter_Backup_'${HOSTNAME}'_'
export outputfilesuffix=${targetversion}'_'${DATEDTGS}
export outputfiletype=.txt
export backupfiletype=.tgz
...
if [ "${sys_type_SMS}" == "true" ]; then
    #
    # Handle SMS Setup for domains
    #
    
    export outputfilefqpn=${outputfilepath}/${outputfileprefix}${outputfilesuffix}${outputfiletype}
    export backupfilefqpn=${outputfilepath}/${outputfileprefix}${outputfilesuffix}$backupfiletype
    
    BackupLogExporterTarget "${outputfilepath}" "${outputfile}fqpn" "$backupfilefqpn"
    
elif [ "${sys_type_MDS}" == "true" ]; then
    #
    # Handle MDSM Setup for domains
    #
    
    # Handle MDS level
    #
    export outputfilefqpn=${outputfilepath}/${outputfileprefix}'_mds_'${outputfilesuffix}${outputfiletype}
    export backupfilefqpn=${outputfilepath}/${outputfileprefix}'_mds_'${outputfilesuffix}$backupfiletype
    ...
    for j in "${DOMAINSARRAY[@]}" ; do
        export sourcedomain="${j}"
        echo 'Process domain : '$sourcedomain | tee -a -i ${logfilepath}
        
        if [ ! -r ${outputfilepath}/$sourcedomain ] ; then
            mkdir -pv ${outputfilepath}/$sourcedomain
            chmod 775 ${outputfilepath}/$sourcedomain
        else
            chmod 775 ${outputfilepath}/$sourcedomain
        fi
        
        export outputfilefqpn=${outputfilepath}/$sourcedomain/${outputfileprefix}'_'$sourcedomain'_'${outputfilesuffix}${outputfiletype}
        export backupfilefqpn=${outputfilepath}/$sourcedomain/${outputfileprefix}'_'$sourcedomain'_'${outputfilesuffix}$backupfiletype
        
        BackupLogExporterTarget "${outputfilepath}/$sourcedomain" "${outputfile}fqpn" "$backupfilefqpn" "$sourcedomain"
        
    done
    
else
    # huh, what?  Not an SMS or MDS... what gives...
    echo "System is not Security Management Server or Multi-Domain Management Server!"
    echo
    echo "This script is meant for SMS or MDSM hosts, exiting!"
    exit 255
