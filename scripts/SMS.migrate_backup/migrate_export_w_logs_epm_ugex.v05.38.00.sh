...
export BASHScriptFileNameRoot=migrate_export_w_logs_epm_ugex
export BASHScriptShortName="migrate_export_epm_w_logs"
export BASHScriptnohupName=${BASHScriptShortName}
export BASHScriptDescription="migrate export EPM with logs and EP Client MSI to local folder using version tools"
...
export outputfileprefix='migrate_export_'${HOSTNAME}'_'${gaiaversion}
...
if [ -z ${CLIparm_l01_toolvername} ]; then
    if ${EXPORTVERSIONDIFFERENT} ; then
        export outputfileprefix='migrate_export_'${HOSTNAME}'_'${gaiaversion}'_export_to_'${toolsversion}
    else
        export outputfileprefix='migrate_export_'${HOSTNAME}'_'${gaiaversion}
    fi
else
    if ${EXPORTVERSIONDIFFERENT} ; then
        export outputfileprefix='migrate_export_'${HOSTNAME}'_'${gaiaversion}'_export_to_'${toolsversion}'_using_'${CLIparm_l01_toolvername}
    else
        export outputfileprefix='migrate_export_'${HOSTNAME}'_'${gaiaversion}'_export_using_'${CLIparm_l01_toolvername}
    fi
fi
...
if [ ! -r ${migratefilepath} ]; then
    #
    # OK, could not find the tools we are looking for
    #
    if ${EXPORTVERSIONDIFFERENT} ; then
        #
        # Target version for migrate|migrate_server export is different so we absolutely need the Upgrade Tools
        #
        if [ -r /opt/CPupgrade-tools-${toolsversion}/${migratefilename} ]; then
            #
            # Maybe use the local copy of upgrade_tools where we expect the migrate file
            #
            export migratefilefolderroot=/opt/CPupgrade-tools-${toolsversion}
            export migratefilepath=${migratefilefolderroot}/
            export migratefile=${migratefilepath}${migratefilename}
            
        else
            #
            # Migrate location is missing
            #
            ...
            echo '  Also Missing folder :  '/opt/CPupgrade-tools-${toolsversion}/${migratefilename} | tee -a -i ${logfilepath}
        ...
        #
        # Target version for migrate|migrate_server export is the same so we start with the Upgrade Tools but can divert to original tools
        #
        if [ -r /opt/CPupgrade-tools-${toolsversion}/${migratefilename} ]; then
            #
            # Maybe use the local copy of upgrade_tools where we expect the migrate file
        ...
        elif [ -r ${FWDIR}/bin/upgrade_tools/${migratefilename} ]; then
            #
            # Maybe use the local one where we expect the migrate file
            #
            export migratefilefolderroot=${MDS_FWDIR}/bin/upgrade_tools
            export migratefilepath=${migratefilefolderroot}/
            export migratefile=${migratefilepath}${migratefilename}
            ...
            echo '  Missing folder      :  '${migratefilepath} | tee -a -i ${logfilepath}
            echo '  Also Missing folder :  '/opt/CPupgrade-tools-${toolsversion}/${migratefilename} | tee -a -i ${logfilepath}
            echo '  And Missing folder  :  '${MDS_FWDIR}/bin/upgrade_tool/${migratefilename} | tee -a -i ${logfilepath}
            echo ' EXITING...' | tee -a -i ${logfilepath}
            echo | tee -a -i ${logfilepath}
...
#
#    ...]# /opt/CPupgrade-tools-R82.10/scripts/migrate_server export
#    
...
#             '-prepare_background'          Prepare upgrade in background.
#                                            Note: option is valid for import and export modes only.
#             '-cancel_prepare'              Cancel upgrade preparation.
#             '-grace_period <minutes>'      Grace period before starting upgrade completion.
#    
...
if $IsMigrateWIndexes ; then
    ...
    #export command2run='export -n -ivw -x 30'
    export command2run='export -n -ivw -x 30 --include-uepm-msi-files'
    #export command2run='export -n -ivw'
else
    # Migrate does not supports export of indexes
    #export command2run='export -n -ivw -l 30'
    export command2run='export -n -ivw -l 30 --include-uepm-msi-files'
    #export command2run='export -n -ivw'
fi
...
if $IsMigrateWIndexes ; then
    # Migrate supports export of indexes
    #export command2run2='export -n -ivw -x 30 --include-uepm-msi-files'
    export command2run2='export -n -ivw -x 30'
    #export command2run2='export -n -ivw --include-uepm-msi-files'
else
    # Migrate does not supports export of indexes
    #export command2run2='export -n -ivw -l 30 --include-uepm-msi-files'
    export command2run2='export -n -ivw -l 30'
    #export command2run2='export -n -ivw --include-uepm-msi-files'
fi
...
#export migratelogfiledate=`date +%Y.%m.%d_%H.%M`
export upgradereporthtmlfiledate=`date +%Y.%m.%d`
case "${gaiaversion}" in
    R82 | R82.10 | R82.20 ) 
        export upgradereporthtmlfilefqfn=${MDS_FWDIR}/log/upgrade_report-${upgradereporthtmlfiledate}_*.*.*.html
        ;;
    R81.20 ) 
        export upgradereporthtmlfilefqfn=${MDS_FWDIR}/log/upgrade_report-${upgradereporthtmlfiledate}_*.*.*.html
        ;;
