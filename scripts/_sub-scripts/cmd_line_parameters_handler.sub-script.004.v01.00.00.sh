...
#
SubScriptTemplateLevel=004
SubScriptVersion=01.00.00
SubScriptDate=2018-09-29

#

export BASHSubScriptVersion=v01x00x00
export SubScriptName=cmd_line_parameters_handler.sub-script.$ScriptTemplateLevel.v$ScriptVersion
export SubScriptShortName="cliparms.$ScriptTemplateLevel"
export SubScriptDescription="Command line parameters handler"
...
if [ x"$BASHScriptTemplateLevel" = x"$SubScriptTemplateLevel" ] ; then
    ...
    echo 'Verify Actions Scripts Version - OK' | tee -a -i $logfilepath
    echo | tee -a -i $logfilepath
else
...
echo | tee -a -i $logfilepath
echo 'SubscriptName:  '$SubScriptName'  Template Version: '$SubScriptTemplateLevel'  Script Version: '$SubScriptVersion | tee -a -i $logfilepath
echo | tee -a -i $logfilepath
...
CheckScriptVerboseOutput () {
    ...
    if [ -z $SCRIPTVERBOSE ] ; then
        ...
        echo "!! Verbose mode not set from shell level" | tee -a -i $logfilepath
        export SCRIPTVERBOSE=false
        echo | tee -a -i $logfilepath
    elif [ x"`echo "$SCRIPTVERBOSE" | tr '[:upper:]' '[:lower:]'`" = x"false" ] ; then
        # Verbose mode set OFF from shell level
        echo "!! Verbose mode set OFF from shell level" | tee -a -i $logfilepath
        export SCRIPTVERBOSE=false
        echo | tee -a -i $logfilepath
    elif [ x"`echo "$SCRIPTVERBOSE" | tr '[:upper:]' '[:lower:]'`" = x"true" ] ; then
        # Verbose mode set ON from shell level
        echo "!! Verbose mode set ON from shell level" | tee -a -i $logfilepath
        export SCRIPTVERBOSE=true
        echo | tee -a -i $logfilepath
        echo 'Script :  '$0 | tee -a -i $logfilepath
        echo 'Verbose mode enabled' | tee -a -i $logfilepath
        echo | tee -a -i $logfilepath
    else
        # Verbose mode set to wrong value from shell level
        echo "!! Verbose mode set to wrong value from shell level >"$SCRIPTVERBOSE"<" | tee -a -i $logfilepath
        echo "!! Settting Verbose mode OFF, pending command line parameter checking!" | tee -a -i $logfilepath
        export SCRIPTVERBOSE=false
        echo | tee -a -i $logfilepath
    fi
...
if [ x"$SCRIPTVERBOSECHECK" = x"true" ] ; then
    ...
    echo "Status of verbose output at start of command line handler: $SCRIPTVERBOSE" | tee -a -i $logfilepath
fi
