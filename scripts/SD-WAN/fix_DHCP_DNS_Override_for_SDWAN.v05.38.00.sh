...
#
# SCRIPT - fix_DHCP_DNS_Override_for_SDWAN - Fix DHCP DNS Override for SD-WAN gateway
#
#
...
export BASHScriptFileNameRoot=fix_DHCP_DNS_Override_for_SDWAN
export BASHScriptShortName=fix_DHCP_DNS_Override_for_SDWAN
export BASHScriptnohupName=${BASHScriptShortName}
export BASHScriptDescription="Fix DHCP DNS Override for SD-WAN gateway"
...
export BASHScriptsFolder="SD-WAN"

export BASHScripttftptargetfolder="SD-WAN"
...
CheckAndUnlockGaiaDB () {
    ...
    echo -n 'Unlock gaia database : ' | tee -a -i ${logfilepath}
...
if ${isitR77version}; then
    echo "This is an R77.X version..." | tee -a -i ${logfilepath}
    export UseR8XAPI=false
    export UseJSONJQ=false
    export UseJSONJQ16=false
elif ${isitR8Xversion}; then
    echo "This is an R80.X version..." | tee -a -i ${logfilepath}
    export UseR8XAPI=${UseR8XAPI}
    export UseJSONJQ=${UseJSONJQ}
    export UseJSONJQ16=${UseJSONJQ16}
else
    echo "This is not an R77.X or R80.X version ????" | tee -a -i ${logfilepath}
fi
...
targetfolder=/var/log/__customer/upgrade_export/SD-WAN
...
# MODIFIED 2026-01-06:01 -

echo | tee -a -i ${logfilepath}
echo ${txtCYAN}${BASHScriptName}${txtDEFAULT}', script version '${txtYELLOW}${ScriptVersion}${txtDEFAULT}', revision '${txtYELLOW}${ScriptRevision}${txtDEFAULT}', subrevision '${txtGREEN}${ScriptSubRevision}${txtDEFAULT}' from '${txtYELLOW}${ScriptDate}${txtDEFAULT}
echo ${BASHScriptName}', script version '${ScriptVersion}', revision '${ScriptRevision}', subrevision '${ScriptSubRevision}' from '${ScriptDate} >> ${logfilepath}
echo ${BASHScriptDescription} | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}

echo 'Date Time Group   :  '${txtCYAN}${DATEDTGS}${txtDEFAULT}
echo 'Date Time Group   :  '${DATEDTGS} >> ${logfilepath}
echo | tee -a -i ${logfilepath}
...
export dhclient_enter_hooks_fqfn=/etc/dhcp/dhclient-enter-hooks
export dhclient_enter_hooks_original_fqfn=${dhclient_enter_hooks_fqfn}.original
export dhclient_enter_hooks_original_now_fqfn=${dhclient_enter_hooks_original_fqfn}.${gaiaversion}.${DATEDTGS}
export dhclient_enter_hooks_modified_now_fqfn=${dhclient_enter_hooks_fqfn}.modified.${gaiaversion}.${DATEDTGS}
export dhclient_enter_hooks_original_exists=false

export changes_file_fqfn=${targetfolder}/append_to_dhclient-enter-hooks.bash

if [ -r "${changes_file_fqfn}" ]; then
    # OK, modification file exists
    cp "${changes_file_fqfn}" "${logfilefolder}/" | tee -a -i ${logfilepath}
else
    # modification file does not exist
    touch ${changes_file_fqfn} | tee -a -i ${logfilepath}
    echo >> ${changes_file_fqfn}
    echo '#******************************************************' >> ${changes_file_fqfn}
    echo '# SD-WAN Fix for DHCP Issue when DNS is set for Internal DNS Servers' >> ${changes_file_fqfn}
    echo '# Gateway      :  '${HOSTNAME} >> ${changes_file_fqfn}
    echo '# Last Modified:  '${DATEYMD}':01' >> ${changes_file_fqfn}
    echo '#' >> ${changes_file_fqfn}
    echo 'export PEERDNS="no"' >> ${changes_file_fqfn}
...
echo '-------------------------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
echo '-------------------------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
cat ${changes_file_fqfn} | tee -a -i ${logfilepath}
echo '-------------------------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
echo '-------------------------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}

if [ -r "${dhclient_enter_hooks_original_fqfn}" ]; then
    # OK, original file exists
    export dhclient_enter_hooks_original_exists=true
else
    cp "${dhclient_enter_hooks_fqfn}" "${dhclient_enter_hooks_original_fqfn}" | tee -a -i ${logfilepath}
    export dhclient_enter_hooks_original_exists=false
fi
cp "${dhclient_enter_hooks_fqfn}" "${dhclient_enter_hooks_original_now_fqfn}" | tee -a -i ${logfilepath}

errorreturn=0

cp "${dhclient_enter_hooks_original_fqfn}" "${logfilefolder}/" | tee -a -i ${logfilepath}
cp "${dhclient_enter_hooks_original_now_fqfn}" "${logfilefolder}/" | tee -a -i ${logfilepath}

cat ${dhclient_enter_hooks_fqfn} | grep "SD-WAN Fix for DHCP"
errorreturn=$?

if [ ${errorreturn} -ne 0 ] ; then
    # File modification not found
    echo `${dtzs}`${dtzsep} 'Required change to '"${dhclient_enter_hooks_fqfn}"' not found, making changes!' | tee -a -i ${logfilepath}
    
    cat ${changes_file_fqfn} >> ${dhclient_enter_hooks_fqfn}
else
    # File modification found
    echo `${dtzs}`${dtzsep} 'Required change to '"${dhclient_enter_hooks_fqfn}"' found, documenting!' | tee -a -i ${logfilepath}
...
echo '-------------------------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
echo '-------------------------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
cat "${dhclient_enter_hooks_fqfn}" | tee -a -i ${logfilepath}
echo '-------------------------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
echo '-------------------------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}

cp "${dhclient_enter_hooks_fqfn}" "${logfilefolder}/" | tee -a -i ${logfilepath}
cp "${dhclient_enter_hooks_fqfn}" "${dhclient_enter_hooks_modified_now_fqfn}" | tee -a -i ${logfilepath}
cp "${dhclient_enter_hooks_modified_now_fqfn}" "${logfilefolder}/" | tee -a -i ${logfilepath}
...
echo | tee -a -i ${logfilepath}
echo 'Target File variants : '${dhclient_enter_hooks_fqfn} | tee -a -i ${logfilepath}
ls -alh --color=auto --group-directories-first ${dhclient_enter_hooks_fqfn}* | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}
echo 'Log File Folder : '${logfilefolder} | tee -a -i ${logfilepath}
ls -alh --color=auto --group-directories-first ${logfilefolder} | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}
