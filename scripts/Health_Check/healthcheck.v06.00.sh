...
check_system()
{
    ...
    printf '<span><b>OS Version - </b></span><b>' >> $html_file
    ...
    if [[ $sys_type == "GATEWAY" || $sys_type == "STANDALONE" || $sys_type == "VSX" ]]; then
        ...
        if [[ -n $fwctlpstat ]]; then
            ...
            if [[ $kernel_memory_failed -eq 0 ]]; then
                ...
                printf "<span>Warning: Kernel memory had $kernel_memory_failed failures.<br>Presence of kmem failed allocations means that some applications did not get memory.<br>This is usually an indication of a memory problem; most commonly a memory shortage.<br>The natural limit is 2GB, since the Kernel is 32bit.</span><br><br>\n" >> $html_file
            else
                check_failed
                printf "Memory,Kernel Memory (kmem),WARNING,\n" >> $csv_log
...
check_cpu()
{
    ...
    if [[ $cpu_wait_warning -eq 1 ]]; then
        ...
        printf "<span>High CPU wait time occurs when the CPU is idle due to the system waiting for an outstanding disk IO request to complete.<br>This indicates your system is probably low on physical memory and is swapping out memory (paging).<br>The CPU is not actually busy if this number is spiking, the CPU is blocked from doing any useful work waiting for an IO event.</span><br><br>\n" >> $html_file
    fi
...
check_misc_messages()
{
    ...
    printf '<tr class="sectionTableBorder"><td class="sectionTableBorder"><p class="paragraphSpacing"><span class="checkNameBlue"><b>Misc. Messages</b></span><br>\n' >> $html_file
    printf '<span><b>Known Issues in Logs - </b></span><b>' >> $html_file
    ...
    if [[ $(grep -i 'Global param: operation failed: Unknown parameter' $messages_tmp_file) ]]; then
        ...
        printf "<span>\"Global param: operation failed: Unknown parameter\" message detected:<br>Defined kernel parameters or their values are not valid.<br>For more information, refer to sk87006.</span><br><br>\n" >> $html_file
    fi
...
check_debugs()
{
    ...
    if [[ $sys_type == "STANDALONE" || $sys_type == "GATEWAY" || $sys_type == "VSX" ]]; then
        ...
        if [[ $debug_flag_check -ne 0 ]]; then
            ...
            printf "</span><br>\n" >> $html_file
            printf "<span>If these modules no longer need debug flags set:<br>Please run \"fw ctl debug 0\" to clear the debug flags.</span><br>\n" >> $html_file
...
check_clusterxl()
{
    ...
    if [[ "$cphaprob_stat" != *"HA module not started."* ]]; then
        ...
        if [[ $failovers_in_the_last_week -ge 1 ]]; then
            ...
            printf "<span>The cluster has failed over $failovers_in_the_last_week time(s) in the last week.</span><br><br>\n" >> $html_file
        else
            check_passed
...
run_remote_checks()
{
    ...
    while read remote_candidate; do
        ...
        if [[ $(grep generic_error $return_details) ]]; then
            ...
            #Find the log files
            remote_log_files=$(echo "$plain_text_results" | grep '/var/log')
...
display_summary()
{
    ...
    #Add color to section headers and category titles before displaying
        ...
        #Clean up sed files created by script
        rm $(find /var/log -iname sed* -mmin -1)
