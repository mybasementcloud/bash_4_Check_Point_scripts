...
export healthcheckscriptfilepath=

export healthcheckscriptgasource=/usr/local/bin/healthcheck.sh
...
if [ -r "${scriptspathroot}/Health_Check/healthcheck.sh" ] ; then
    ...
    export healthcheckscriptfilepath=${scriptspathroot}/Health_Check/healthcheck.sh
    ...
    export healthcheckscriptfilepath=${scriptsbase}/healthcheck
    ...
    export healthcheckscriptfilepath=${scriptsrepository}/healthcheck
    
elif [ -r "${healthcheckscriptgasource}" ] ; then
    
    echo 'Found GA healthcheck script here : '${healthcheckscriptgasource} | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    # Using . to run the script causes it to exit the complete operation and does not finiish this calling script
    #. ${scriptsrepository}/healthcheck
    export healthcheckscriptfilepath=${healthcheckscriptgasource}
...
if [ -r ${healthcheckscriptgasource} ] ; then
    # check if the version in the GA location is different from what we have after an update
    ${healthcheckscriptgasource} -u | tee -a -i ${logfilepath}
    
    if [ x"${healthcheckscriptfilepath}" == x"" ] ; then
    ...
    healthcheckgaversion=`${healthcheckscriptgasource} -v | grep "Version"`
    healthcheckscriptsversion=`${healthcheckscriptfilepath} | grep "Version"`
    
    if [ ${healthcheckgaversion} != ${healthcheckscriptsversion} ] ; then
        export healthcheckscriptfilepath=${healthcheckscriptgasource}
    else
        export healthcheckscriptfilepath=${healthcheckscriptfilepath}
    fi
fi
...
echo | tee -a -i ${logfilepath}
echo 'Using healthcheck script at '${healthcheckscriptfilepath}' ...' | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}

${healthcheckscriptfilepath}
