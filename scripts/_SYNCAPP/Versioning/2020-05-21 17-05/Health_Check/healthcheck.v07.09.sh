...
r8030_ga_jumbo="140"
r8040_ga_jumbo="0"
latest_cpinfo_build="914000202"
latest_cpuse_build="1858"
...
check_magic_mac()
{
    ...
    elif [[ $current_version -le 8030 ]]; then
...
check_cp_software()
{
    ...
    elif [[ $current_version -ge "8010" ]]; then
        eos_date="31 May 2021"
    elif [[ $current_version -ge "8020" ]]; then
        eos_date="30 September 2022"
    elif [[ $current_version -ge "8030" ]]; then
        eos_date="30 September 2022"
    else
...
check_updates()
{
    ...
    if [[ $(ping -c1 8.8.8.8 2> /dev/null | grep -ow "1 received") ]]; then
        ...
        if [[ -n $curl_cmd ]]; then
            ...
            #$curl_cmd -o $r80_40_temp 'https://supportcenter.checkpoint.com/supportcenter/portal?eventSubmit_doGoviewsolutiondetails=&solutionid=sk153152' --insecure > /dev/null 2>&1
            ...
            #online_latest_R80_40=$(grep -i latest $r80_40_temp | grep -Eiow 'take_[0-9]{1,3}' | head -n1 | grep -o '[0-9]{1,3}')
            online_latest_R80_40="0"
...
run_remote_checks()
{
    ...
    #Check to see if the SMS is active
        ...
        logger "$current_cma is not the current active Management Server for $gateway_name."s
        logger "Please run this script on the Active Management Server and try again."
        return
    fi
    ...
    while read remote_candidate; do
        ...
        if [[ ! -s $cpridAvailCheckResult ]] || [[ "$(cat $cpridAvailCheckResult 2>/dev/null)" =~ .*NULL[[:space:]]?BUF.* ]]; then
            ...
            if [[ $(grep generic_error $return_details) ]]; then
                ...
                logger "Data for following candidate successfully collected: $gateway_name"
...
display_summary()
{
    ...
    #Add color to section headers and category titles before displaying
        ...
        rm $(find /var/log -mmin -1 | grep sed) > /dev/null 2>&1
        rm $(find ./ -mmin -1 | grep sed) > /dev/null 2>&1
