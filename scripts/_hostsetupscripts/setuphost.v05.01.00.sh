...
export OutputDTGTZinUTC=true


# -------------------------------------------------------------------------------------------------
# Early Operational Variables
# -------------------------------------------------------------------------------------------------
...
if [ -z ${JQ} ]; then
    ...
    platform_release_version=`cat /etc/cp-release | cut -c 13- | sed 's/^ *//g' | sed 's/\s*$//g'`
    platform_release_version=${platform_release_version#*R*}
...
export gaia_version=${platform_release_version}
