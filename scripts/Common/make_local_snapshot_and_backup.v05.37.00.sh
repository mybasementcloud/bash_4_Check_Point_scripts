...
#
# SCRIPT Make local Gaia snapshot and Gaia backup
#
#
...
export BASHScriptFileNameRoot=make_local_snapshot_and_backup
export BASHScriptShortName=local_snapshot_and_backup
export BASHScriptnohupName=${BASHScriptShortName}
export BASHScriptDescription="Make local aia snapshot and Gaia backup"
...
export snapshotsfilepath=${customerworkpathroot}/snapshots

if [ ! -r ${snapshotsfilepath} ] ; then
    mkdir -pv ${snapshotsfilepath}
    chmod 775 ${snapshotsfilepath}
else
    chmod 775 ${snapshotsfilepath}
fi
...
# -------------------------------------------------------------------------------------------------
# CheckIfSnapshotNameUsed - Check if target Snapshot Name is already used
# -------------------------------------------------------------------------------------------------

# MODIFIED 2021-05-30 -\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
#

CheckIfSnapshotNameUsed () {
    #
    # CheckIfSnapshotNameUsed - Check if target Snapshot Name is already used
    #
    
    checksnapshotname=$1
    checksnapshotname=${checksnapshotname//\"}
    
    echo | tee -a -i ${logfilepath}
    echo 'Check if Snapshot '${checksnapshotname}' exists' | tee -a -i ${logfilepath}
    
    export snapshotexists=false
    export checksnapshotdate=`clish -c "show snapshot ${checksnapshotname} date"`
    export doessnapshotdateexist=`test -z "${checksnapshotdate}" ; echo $?`
    echo 'Snapshot date for '${checksnapshotname}' = "'${checksnapshotdate}'"' | tee -a -i ${logfilepath}
    if [ ${doessnapshotdateexist} -eq 1 ]; then 
        export snapshotexists=true
        echo 'Snapshot '${checksnapshotname}' exists' | tee -a -i ${logfilepath}
    else
        export snapshotexists=false
        echo 'Snapshot '${checksnapshotname}' DOES NOT exist' | tee -a -i ${logfilepath}
    fi
...
# -------------------------------------------------------------------------------------------------
# END:  CheckIfSnapshotNameUsed - Check if target Snapshot Name is already used
# -------------------------------------------------------------------------------------------------
...
case "${gaiaversion}" in
    R80 ) 
        export snapshotnameversion=R80x00
        ;;
    R80.10 ) 
        export snapshotnameversion=R80x10
        ;;
    R80.20.M1 ) 
        export snapshotnameversion=R80x21
        ;;
    R80.20.M2 ) 
        export snapshotnameversion=R80x22
        ;;
    R80.20 ) 
        export snapshotnameversion=R80x20
        ;;
    R80.30 ) 
        export snapshotnameversion=R80x30
        ;;
    R80.40 ) 
        export snapshotnameversion=R80x40
        ;;
    R81 ) 
        export snapshotnameversion=R81x00
        ;;
    R81.10 ) 
        export snapshotnameversion=R81x10
        ;;
    R81.20 ) 
        export snapshotnameversion=R81x20
        ;;
    R82 ) 
        export snapshotnameversion=R82x00
        ;;
    R82.10 ) 
        export snapshotnameversion=R82x10
        ;;
    R82.20 ) 
        export snapshotnameversion=R82x20
        ;;
    *)
        export snapshotnameversion=${gaiaversion//./x}
        ;;
esac

export snapshotnamedate=`date +%Y%m%d`
export snapshotname=${snapshotnamedate}${snapshotnameversion}

echo | tee -a -i ${logfilepath}
printf "%-30s : %s\n" 'snapshotnamedate' "${snapshotnamedate}" | tee -a -i ${logfilepath}
printf "%-30s : %s\n" 'snapshotname (initial)' "${snapshotname}" | tee -a -i ${logfilepath}

export snapshotexists=false

echo 'Check if Snapshot with name '${snapshotname}' already exists...' | tee -a -i ${logfilepath}
CheckIfSnapshotNameUsed ${snapshotname}

if ${snapshotexists} ; then
    # the Snapshot name we currently have is already used
    echo 'Snapshot with name '${snapshotname}' already exists, trying alternatives...' | tee -a -i ${logfilepath}
    
    if ${snapshotexists} ; then
        for namextension in {0..9} ; do
            checksnapshotname=${snapshotname}${namextension}
            CheckIfSnapshotNameUsed ${checksnapshotname}
            if ! ${snapshotexists} ; then
    ...
    export snapshotname=${checksnapshotname}
    echo 'Snapshot with name '${snapshotname}' does not exist, continueing...' | tee -a -i ${logfilepath}
else
    # the Snapshot name we currently have is NOT already used
    echo 'Snapshot with name '${snapshotname}' does not exist, continueing...' | tee -a -i ${logfilepath}
...
echo | tee -a -i ${logfilepath}
printf "%-30s : %s\n" 'snapshotnamedate' "${snapshotnamedate}" | tee -a -i ${logfilepath}
printf "%-30s : %s\n" 'snapshotname (final)' "${snapshotname}" | tee -a -i ${logfilepath}
...
#[Expert@CORE-G3-Mgmt-01:0]# cpinfo -y all > cpinfo_y_all.txt ; echo ; cat cpinfo_y_all.txt | grep "JUMBO_HF_MAIN" | head -n 1

#This is Check Point CPinfo Build 914000215 for GAIA

        #HOTFIX_R81_JUMBO_HF_MAIN        Take:  25
#[Expert@CORE-G3-Mgmt-01:0]# cpinfo -y all > cpinfo_y_all.txt ; echo ; cat cpinfo_y_all.txt | grep "JUMBO_HF_MAIN" | head -n 1 | cut -d " " -f 3

#This is Check Point CPinfo Build 914000215 for GAIA

#25
#[Expert@CORE-G3-Mgmt-01:0]#
#[Expert@CORE-G3-Mgmt-01:0]# clish -c "show version all" | grep -w "OS build"
#OS build 392
#[Expert@CORE-G3-Mgmt-01:0]#

cpinfo -y all > /var/log/tmp/cpinfo_y_all.txt

export snapshotdescriptionJHF=`cat /var/log/tmp/cpinfo_y_all.txt | grep "JUMBO_HF_MAIN" | head -n 1 | cut -d " " -f 3`
export snapshotdescriptionbuild=`clish -c "show version all" | grep -w "OS build" | cut -d " " -f 3`
export snapshotdescriptiondate=`date +%Y-%m-%d`
export snapshotdescriptionversion=${gaiaversion}

echo | tee -a -i ${logfilepath}
printf "%-30s : %s\n" 'snapshotdescriptionJHF' "${snapshotdescriptionJHF}" | tee -a -i ${logfilepath}
printf "%-30s : %s\n" 'snapshotdescriptionbuild' "${snapshotdescriptionbuild}" | tee -a -i ${logfilepath}
printf "%-30s : %s\n" 'snapshotdescriptiondate' "${snapshotdescriptiondate}" | tee -a -i ${logfilepath}
printf "%-30s : %s\n" 'snapshotdescriptionversion' "${snapshotdescriptionversion}" | tee -a -i ${logfilepath}

#export snapshotdescription="${snapshotdescriptionversion} Take ${snapshotdescriptionbuild} JHF ${snapshotdescriptionJHF} ${snapshotdescriptiondate} for Export Backup"
export snapshotdescription="${snapshotdescriptionversion}_Take_${snapshotdescriptionbuild}_JHF_${snapshotdescriptionJHF}_${snapshotdescriptiondate}_for_Export_Backup"

echo | tee -a -i ${logfilepath}
printf "%-30s : %s\n" 'snapshotdescription' "${snapshotdescription}" | tee -a -i ${logfilepath}

rm /var/log/tmp/cpinfo_y_all.txt | tee -a -i ${logfilepath}

#printf "%-30s : %s\n" 'CSVEXPORT05TAGS' "${CSVEXPORT05TAGS}" | tee -a -i ${logfilepath}

export snapshotfilepath=${customerworkpathroot}/snapshots

echo | tee -a -i ${logfilepath}
printf "%-30s : %s\n" 'snapshotfilepath' "${snapshotfilepath}" | tee -a -i ${logfilepath}
...
#if [ -r ${jsoncontrolfilefqpn} ] ; then
    # # Central location of json control files has the target_ftp_host.json file
    #export ftpexportipv4address=`cat ${jsoncontrolfilefqpn} | ${JQ} .targethost.ipv4address`
    #export ftpexportipv4address=${ftpexportipv4address//\"}
    #export ftpexportpathroot=`cat ${jsoncontrolfilefqpn} | ${JQ} .targethost.ftppathroot`
    #export ftpexportpathroot=${ftpexportpathroot//\"}
    #export ftpexportpath=${ftpexportpathroot}/${HOSTNAME}/
    #export ftpexportusername=`cat ${jsoncontrolfilefqpn} | ${JQ} .targethost.ftpusername`
    #export ftpexportusername=${ftpexportusername//\"}
    #export ftpexportpassword=`cat ${jsoncontrolfilefqpn} | ${JQ} .targethost.ftpuserpassword`
    #temp="${ftpexportpassword%\"}" ; export ftpexportpassword="${temp#\"}"
    # # " fix metafile display
    
#elif [ -r ${jsoncontrolfilesourcefqpn} ] ; then
    # # Alternate location of json control files has the target_ftp_host.json file
    #export ftpexportipv4address=`cat ${jsoncontrolfilesourcefqpn} | ${JQ} .targethost.ipv4address`
    #export ftpexportipv4address=${ftpexportipv4address//\"}
    #export ftpexportpathroot=`cat ${jsoncontrolfilesourcefqpn} | ${JQ} .targethost.ftppathroot`
    #export ftpexportpathroot=${ftpexportpathroot//\"}
    #export ftpexportpath=${ftpexportpathroot}/${HOSTNAME}/
    #export ftpexportusername=`cat ${jsoncontrolfilesourcefqpn} | ${JQ} .targethost.ftpusername`
    #export ftpexportusername=${ftpexportusername//\"}
    #export ftpexportpassword=`cat ${jsoncontrolfilesourcefqpn} | ${JQ} .targethost.ftpuserpassword`
    #temp="${ftpexportpassword%\"}" ; export ftpexportpassword="${temp#\"}"
    # # " fix metafile display
    
#else
    # # Central location of json control files does not have the target_ftp_host.json file so ask for input
    #read -p "Enter IPv4 Address (w.x.y.z) : " ftpexportipv4address
    #read -p "Enter root path on FTP host : " ftpexportpathroot
    #export ftpexportpath=${ftpexportpathroot}/${HOSTNAME}/
    #read -p "Enter Password for FTP Access : " ftpexportusername
    #read -p "Enter Username for FTP Access : " -s ftpexportpassword
    
#fi

#if [ x"${ftpexportusername}" == x"" ] ; then
    #read -p "Enter Password for FTP Access : " ftpexportusername
#fi
#if [ x"${ftpexportpassword}" == x"" ] ; then
    #read -p "Enter Username for FTP Access : " -s ftpexportpassword
#fi

#printf "%-30s : %s\n" 'ftpexportipv4address' "${ftpexportipv4address}" | tee -a -i ${logfilepath}
#printf "%-30s : %s\n" 'ftpexportpathroot' "${ftpexportpathroot}" | tee -a -i ${logfilepath}
#printf "%-30s : %s\n" 'ftpexportpath' "${ftpexportpath}" | tee -a -i ${logfilepath}
#printf "%-30s : %s\n" 'ftpexportusername' "${ftpexportusername}" | tee -a -i ${logfilepath}
#printf "%-30s : %s\n" 'ftpexportpassword' '*****' | tee -a -i ${logfilepath}
#echo | tee -a -i ${logfilepath}
...
export ccommand="show snapshots"
echo 'Execute CLI Command :  clish -c "'${ccommand}'"' | tee -a -i ${logfilepath}
clish -c "${ccommand}" | tee -a -i ${logfilepath}
errorreturn=$?
echo | tee -a -i ${logfilepath}
echo 'CLI Command error :  errorreturn = '${errorreturn} | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}
...
case "${gaiaversion}" in
    R82 | R82.10 | R82.20 | R81 | R81.10 | R81.20 ) 
        # The original add snapshot command was deprecated, without a clear replacement
        #
        #export ccommand="add snapshot ${snapshotname} desc '"${snapshotdescription}"'"
        #
        # The new add snapshot-onetime with target local has the problem that it REQUIRES the local path 
        # and does not retain the snapshot in the list of snapshots, instead placing the tar version into
        # the path VALUE, this does not help with the follow on export action, which fails
        #
        #add snapshot-onetime name VALUE description VALUE target local path VALUE
        #export ccommand="add snapshot-onetime name ${snapshotname} description '"${snapshotdescription}"' target local path "${snapshotsfilepath}
        #echo 'Execute CLI Command :  clish -c "'${ccommand}'"' | tee -a -i ${logfilepath}
        #clish -c "${ccommand}" | tee -a -i ${logfilepath}
        #echo | tee -a -i ${logfilepath}
        
        #add snapshot-onetime name VALUE description VALUE target ftp path VALUE ip VALUE username VALUE password VALUE
        #export ccommand="add snapshot-onetime name ${snapshotname} description '"${snapshotdescription}"' target ftp path ${ftpexportpath} ip ${ftpexportipv4address} username ${ftpexportusername} password ${ftpexportpassword}"
        export ccommand="add snapshot-onetime name ${snapshotname} description '"${snapshotdescription}"'"
        echo 'Execute '"${gaiaversion}"' CLI Command :  clish -c "'${ccommand}'"' | tee -a -i ${logfilepath}
        clish -c "${ccommand}" | tee -a -i ${logfilepath}
        errorreturn=$?
        echo | tee -a -i ${logfilepath}
        echo 'CLI Command error :  errorreturn = '${errorreturn} | tee -a -i ${logfilepath}
        ...
        ;;
    R80 | R80.10 | R80.20.M1 | R80.20.M2 | R80.20 | R80.30 | R80.40 ) 
        # R80.X original add snapshot command
        #
        #export ccommand="add snapshot ${snapshotname} desc '"${snapshotdescription}"'"
        #
        export ccommand="add snapshot ${snapshotname} desc '"${snapshotdescription}"'"
        ...
        ;;
    *)
        # R77.X original add snapshot command
        #
        #export ccommand="add snapshot ${snapshotname} desc '"${snapshotdescription}"'"
        #
        export ccommand="add snapshot ${snapshotname} desc '"${snapshotdescription}"'"
...
export ccommand="show backup status"
echo 'Execute CLI Command :  clish -c "'${ccommand}'"' | tee -a -i ${logfilepath}
clish -c "${ccommand}" | tee -a -i ${logfilepath}
errorreturn=$?
echo | tee -a -i ${logfilepath}
...
export ccommand="'show snapshots'"
export wcommand="echo ; echo ; echo 'CTRL-C when snapshot "${snapshotname}" export complete' ; echo ; clish -c "${ccommand}
export ccommand2="'show backup status'"
export wcommand=${wcommand}" ; echo ; echo 'Backup Status:'; clish -c "${ccommand2}
echo 'Execute CLI Command :  watch -d -n 1 "'${wcommand}'"' | tee -a -i ${logfilepath}
watch -d -n 5 "${wcommand}"
echo | tee -a -i ${logfilepath}
...
#case "${gaiaversion}" in
    #R82 | R82.10 | R82.20 | R81 | R81.10 | R81.20 ) 
        # #set snapshot-onetime export VALUE target ftp path VALUE ip VALUE username VALUE password VALUE
        # #clish -c "set snapshot-onetime export ${snapshotname} target ftp path ${ftpexportpath} ip ${ftpexportipv4address} username ${ftpexportusername} password ${ftpexportpassword}" | tee -a -i ${logfilepath}
        # #export ccommand="set snapshot-onetime export ${snapshotname} target ftp path ${ftpexportpath} ip ${ftpexportipv4address} username ${ftpexportusername} password ${ftpexportpassword}"
        
        # # Since the new add snapshot-onetime executing locally does not leave the snapshot in place
        # # we have to use the create and export variant
        # #
        # #set snapshot-onetime forced target ftp name VALUE path VALUE ip VALUE username VALUE password VALUE
        # #clish -c "set snapshot-onetime forced target ftp name ${snapshotname} path ${ftpexportpath} ip ${ftpexportipv4address} username ${ftpexportusername} password ${ftpexportpassword}" | tee -a -i ${logfilepath}
        # #export ccommand="set snapshot-onetime forced target ftp name ${snapshotname} path ${ftpexportpath} ip ${ftpexportipv4address} username ${ftpexportusername} password ${ftpexportpassword}"
        ...
        #;;
    #R80 | R80.10 | R80.20.M1 | R80.20.M2 | R80.20 | R80.30 | R80.40 ) 
        # # R80.X original set snapshot export command
        # # set snapshot export VALUE path VALUE name VALUE
        # #
        # #export ccommand="add snapshot ${snapshotname} desc '"${snapshotdescription}"'"
        # #
        #export ccommand="set snapshot export ${snapshotname} path ${snapshotfilepath} name ${snapshotname}.tar"
        #echo 'Execute '"${gaiaversion}"' CLI Command :  clish -c "'${ccommand}'"' | tee -a -i ${logfilepath}
        #clish -c "${ccommand}" | tee -a -i ${logfilepath}
        #echo | tee -a -i ${logfilepath}
        #;;
    #*)
        # # R77.X original set snapshot export command
        # # set snapshot export VALUE path VALUE name VALUE
        # #
        # #export ccommand="add snapshot ${snapshotname} desc '"${snapshotdescription}"'"
        # #
...
#watch -d -n 1 "echo ; echo ; echo 'CTRL-C when snapshot export complete!' ; echo ; clish -c 'show snapshots'"
#export ccommand="'show snapshots'"
#export wcommand="echo ; echo ; echo 'CTRL-C when snapshot "${snapshotname}" export complete' ; echo ; clish -c "${ccommand}
#echo 'Execute CLI Command :  watch -d -n 1 "'${wcommand}'"' | tee -a -i ${logfilepath}
#watch -d -n 5 "${wcommand}"
...
#clish -c "add backup ftp ip ${ftpexportipv4address} path ${ftpexportpath} username ${ftpexportusername} password ${ftpexportpassword} interactive" | tee -a -i ${logfilepath}
#export ccommand="add backup ftp ip ${ftpexportipv4address} path ${ftpexportpath} username ${ftpexportusername} password ${ftpexportpassword} interactive"
export ccommand="add backup local interactive"
echo 'Execute CLI Command :  clish -c "'${ccommand}'"' | tee -a -i ${logfilepath}
clish -c "${ccommand}" | tee -a -i ${logfilepath}
errorreturn=$?
echo | tee -a -i ${logfilepath}
