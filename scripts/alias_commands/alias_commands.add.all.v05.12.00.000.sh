...
alias vnamenow='echo ${HOSTNAME}.`CPRELEASEVERSION`.`DTGSDATE`'
...
printf "${tCYAN}%-30s${tNORM} : %s\n" "vnamenow" 'Version Name Now - Generate hostname . (dot) Check Point release version . (dot) Date Time Group with Seconds :  ${HOSTNAME}.release_version.YYYY-mm-dd-HHMMSSTZ3' >> ${tempENVHELPFILEalias}
...
alias collect_core_dumps_now='DTGSNOW=`DTGSDATE`;WORKNOWFOLDER="${MYWORKFOLDERDUMP}/$DTGSNOW";mkdir -pv "${WORKNOWFOLDER}";list "${MYWORKFOLDERDUMP}/";echo;cd "${WORKNOWFOLDER}";echo;echo Current path = `pwd`;echo;echo "Core dumps:";echo;ls -alh --color=auto /var/log/dump/usermode/;echo;echo "Copy core dump *.gz files to this folder";echo;cp /var/log/dump/usermode/*.gz .;echo;ls -alh --color=auto .;echo'
...
if [ -n ${MYTFTPSERVER} ] ; then
    ...
    alias getupdatetoolsscript 2> /dev/null > /dev/null ; if [ $? -eq 0 ] ; then echo 'alias getupdatetoolsscript SET!  REMOVING!'; unalias getupdatetoolsscript; else echo 'getupdatetoolsscript NOT aliased'; fi ; echo
...
alias generate_script_links='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/generate_script_links.v05.12.00.sh'
alias remove_script_links='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/remove_script_links.v05.12.00.sh'
alias reset_script_links='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/remove_script_links.v05.12.00.sh;echo;${MYWORKFOLDERUGEXSCRIPTS}/generate_script_links.v05.12.00.sh;echo;list'
...
alias make_snapshot_and_backup_to_ftp='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/Common/make_snapshot_and_backup_to_ftp_target.v05.12.00.sh'
printf "${tCYAN}%-30s${tNORM} : %s\n" "make_snapshot_and_backup_to_ftp" 'Make Gaia Snapshot and Backup and export to FTP target' >> ${tempENVHELPFILEalias}

alias showsnapshots='clish -c "show snapshots"'
printf "${tCYAN}%-30s${tNORM} : %s\n" "showsnapshots" 'Show current list of Gaia Snapshots' >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-30s${tNORM} : %s\n" "watchsnapshots" 'Watch current list of Gaia Snapshots' >> ${tempENVHELPFILEalias}
...
alias alias_commands_add_user='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/UserConfig/alias_commands_add_user.v05.12.00.sh'
alias alias_commands_add_all_users='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/UserConfig/alias_commands_add_all_users.v05.12.00.sh'
alias alias_commands_update_user='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/UserConfig/alias_commands_update_user.v05.12.00.sh'
alias alias_commands_update_all_users='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/UserConfig/alias_commands_update_all_users.v05.12.00.sh'
