...
# =============================================================================
# JSON Query JQ and version specific JQ16, JQ181 values
# =============================================================================
...
#export JQPATH=${MYWORKFOLDER}/_tools/JQ
export JQPATH=${MYWORKFOLDER}/_tools/JQ
...
if [ -r ${JQFQFN} ] ; then
    ...
    echo "jq-linux64 or jq, found as ${JQFQFN}" | tee -a -i ${logfilepath}
elif [ -r "./_tools/JQ/${JQFILE}" ] ; then
    # OK we have the local folder alternative
    export JQPATH=./_tools/JQ
    export JQFILE=jq-linux64
    export JQ=${JQPATH}/${JQFILE}
    export JQNotFound=false
    export UseJSONJQ=true
    export JQFQFN=${JQPATH}/${JQFILE}
    echo "jq-linux64 or jq, found as ${JQFQFN}" | tee -a -i ${logfilepath}
    ...
    export UseJSONJQ=false
    export JQFQFN=
    echo "JQ NOT found!" | tee -a -i ${logfilepath}
fi
...
export JQ16PATH=${MYWORKFOLDER}/_tools/JQ_v01.06.00
...
if [ -r ${JQ16FQFN} ] ; then
    ...
    echo "JQ v 1.6 found as jq-linux64 or jq, at ${JQ16FQFN}" | tee -a -i ${logfilepath}
elif [ -r "./_tools/JQ_v01.06.00/${JQ16FILE}" ] ; then
    # OK we have the local folder alternative
    export JQ16PATH=./_tools/JQ_v01.06.00
    export JQ16=${JQ16PATH}/${JQ16FILE}
    export JQ16NotFound=false
    export UseJSONJQ16=true
    export JQ16FQFN=${JQ16PATH}/${JQ16FILE}
    echo "JQ v 1.6 found as jq-linux64 or jq, at ${JQ16FQFN}" | tee -a -i ${logfilepath}
    ...
    export UseJSONJQ16=false
    export JQ16FQFN=
    echo "JQ v 1.6 NOT found!" | tee -a -i ${logfilepath}
fi
...
#export JQ181PATH=${MYWORKFOLDER}/_tools/JQ_v01.08.01
export JQ181PATH=${MYWORKFOLDER}/_tools/JQ_v01.08.01
...
if [ -r ${JQ181FQFN} ] ; then
    ...
    echo "JQ v 1.8.1 found as jq-linux64 or jq, at ${JQ181FILE}" | tee -a -i ${logfilepath}
elif [ -r "./_tools/JQ_v01.08.01/${JQ181FILE}" ] ; then
    # OK we have the local folder alternative
    export JQ181PATH=./_tools/JQ_v01.08.01
    export JQ181=${JQ181PATH}/${JQ181FILE}
    export JQ181NotFound=false
    export UseJSONJQ181=true
    export JQ181FQFN=${JQ181PATH}/${JQ181FILE}
    echo "JQ v 1.8.1 found as jq-linux64 or jq, at ${JQ181FILE}" | tee -a -i ${logfilepath}
    ...
    export UseJSONJQ181=false
    export JQ181FQFN=
    echo "JQ v 1.8.1 NOT found!" | tee -a -i ${logfilepath}
fi

printf "variable :  ${tCYAN}%-25s${tNORM} = %s\n" "JQ" ${JQ} >> ${tempENVHELPFILEvars}
printf "variable :  ${tCYAN}%-25s${tNORM} = %s\n" "JQPATH" ${JQPATH} >> ${tempENVHELPFILEvars}
printf "variable :  ${tCYAN}%-25s${tNORM} = %s\n" "JQFILE" ${JQFILE} >> ${tempENVHELPFILEvars}
printf "variable :  ${tCYAN}%-25s${tNORM} = %s\n" "JQFQFN" ${JQFQFN} >> ${tempENVHELPFILEvars}
...
printf "variable :  ${tCYAN}%-25s${tNORM} = %s\n" "JQ181" ${JQ181} >> ${tempENVHELPFILEvars}
printf "variable :  ${tCYAN}%-25s${tNORM} = %s\n" "JQ181PATH" ${JQ181PATH} >> ${tempENVHELPFILEvars}
printf "variable :  ${tCYAN}%-25s${tNORM} = %s\n" "JQ181FILE" ${JQ181FILE} >> ${tempENVHELPFILEvars}
printf "variable :  ${tCYAN}%-25s${tNORM} = %s\n" "JQ181FQFN" ${JQ181FQFN} >> ${tempENVHELPFILEvars}
echo >> ${tempENVHELPFILEvars}
...
printf "${tCYAN}%-35s${tNORM} : %s\n" "CPVERSIONNOW" 'Generate Check Point release version . (dot) Date Time Group :  release_version.YYYY-mm-dd-HHMMTZ3 (Altnernate)' >> ${tempENVHELPFILEalias}
...
printf "${tCYAN}%-35s${tNORM} : %s\n" "CPVERSIONHOSTNOW" 'Generate Check Point release version . (dot) hostname . (dot) Date Time Group with Seconds :  release_version.${HOSTNAME}.YYYY-mm-dd-HHMMSSTZ3 (Altnernate)' >> ${tempENVHELPFILEalias}
...
printf "${tCYAN}%-35s${tNORM} : %s\n" "HOSTCPVERSIONNOW" 'Generate hostname . (dot) Check Point release version . (dot) Date Time Group with Seconds :  ${HOSTNAME}.release_version.YYYY-mm-dd-HHMMSSTZ3 (Altnernate)' >> ${tempENVHELPFILEalias}
...
#alias configdump='echo;gougex;pwd;echo;echo;./config_capture;echo;echo'
alias configdump='echo;config_capture;echo;runHCP;echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "configdump" 'Execute configuration dump (config_capture), HCP (runHCP)' >> ${tempENVHELPFILEalias}
...
alias braindump='echo;config_capture;echo;interface_info;echo;check_point_service_status_check;echo;runHCP;echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "braindump" 'Execute complete configuration, status, health dumps (config_capture, interface_info, check_point_service_status_check, runHCP) ' >> ${tempENVHELPFILEalias}

# 2022-03-23
alias systemhealthdump='echo;echo;runHCP;echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "systemhealthdump" 'Execute health status check HCP (runHCP)' >> ${tempENVHELPFILEalias}

#========================================================================================
# 2026-01-20:01

alias makeconf_files='echo;mkdir ${MYWORKFOLDERUGEX}/conf_files;echo;list ${MYWORKFOLDERUGEX}/conf_files;echo'
alias clearconf_files='echo;list ${MYWORKFOLDERUGEX}/conf_files;echo;rm -frdv ${MYWORKFOLDERUGEX}/conf_files;echo;mkdir ${MYWORKFOLDERUGEX}/conf_files;echo;list ${MYWORKFOLDERUGEX}/conf_files;echo'
alias cleanconf_files='echo;list ${MYWORKFOLDERUGEX}/conf_files;echo;rm -frdv ${MYWORKFOLDERUGEX}/conf_files;echo;list ${MYWORKFOLDERUGEX};echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "makeconf_files" 'Create the conf_files folder '${MYWORKFOLDERUGEX}/conf_files >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "clearconf_files" 'Remove and Remake the conf_files folder '${MYWORKFOLDERUGEX}/conf_files >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "cleanconf_files" 'Remove the conf_files folder '${MYWORKFOLDERUGEX}/conf_files >> ${tempENVHELPFILEalias}

alias makedump='echo;mkdir ${MYWORKFOLDERUGEX}/dump;echo;list ${MYWORKFOLDERUGEX}/dump;echo'
alias cleardump='echo;list ${MYWORKFOLDERUGEX}/dump;echo;rm -frdv ${MYWORKFOLDERUGEX}/dump;echo;mkdir ${MYWORKFOLDERUGEX}/dump;echo;list ${MYWORKFOLDERUGEX}/dump;echo'
alias cleandump='echo;list ${MYWORKFOLDERUGEX}/dump;echo;rm -frdv ${MYWORKFOLDERUGEX}/dump;echo;list ${MYWORKFOLDERUGEX};echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "makedump" 'Create the dump folder '${MYWORKFOLDERUGEX}/dump >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "cleardump" 'Remove and Remake the dump folder '${MYWORKFOLDERUGEX}/dump >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "cleandump" 'Remove the dump folder '${MYWORKFOLDERUGEX}/dump >> ${tempENVHELPFILEalias}

alias makehomebackup='echo;mkdir ${MYWORKFOLDERUGEX}/home.backup;echo;list ${MYWORKFOLDERUGEX}/home.backup;echo'
alias clearhomebackup='echo;list ${MYWORKFOLDERUGEX}/home.backup;echo;rm -frdv ${MYWORKFOLDERUGEX}/home.backup;echo;mkdir ${MYWORKFOLDERUGEX}/home.backup;echo;list ${MYWORKFOLDERUGEX}/home.backup;echo'
alias cleanhomebackup='echo;list ${MYWORKFOLDERUGEX}/home.backup;echo;rm -frdv ${MYWORKFOLDERUGEX}/home.backup;list ${MYWORKFOLDERUGEX};echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "makehomebackup" 'Create the home.backup folder '${MYWORKFOLDERUGEX}/home.backup >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "clearhomebackup" 'Remove and Remake the home.backup folder '${MYWORKFOLDERUGEX}/home.backup >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "cleanhomebackup" 'Remove the home.backup folder '${MYWORKFOLDERUGEX}/home.backup >> ${tempENVHELPFILEalias}

alias makehealthchecks='echo;mkdir ${MYWORKFOLDERUGEX}/healthchecks;echo;list ${MYWORKFOLDERUGEX}/healthchecks;echo'
alias clearhealthchecks='echo;list ${MYWORKFOLDERUGEX}/healthchecks;echo;rm -frdv ${MYWORKFOLDERUGEX}/healthchecks;echo;mkdir ${MYWORKFOLDERUGEX}/healthchecks;echo;list ${MYWORKFOLDERUGEX}/healthchecks;echo'
alias cleanhealthchecks='echo;list ${MYWORKFOLDERUGEX}/healthchecks;echo;rm -frdv ${MYWORKFOLDERUGEX}/healthchecks;list ${MYWORKFOLDERUGEX};echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "makehealthchecks" 'Create the healthchecks folder '${MYWORKFOLDERUGEX}/healthchecks >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "clearhealthchecks" 'Remove and Remake the healthchecks folder '${MYWORKFOLDERUGEX}/healthchecks >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "cleanhealthchecks" 'Remove the healthchecks folder '${MYWORKFOLDERUGEX}/healthchecks >> ${tempENVHELPFILEalias}

alias makehost_data='echo;mkdir ${MYWORKFOLDERUGEX}/host_data;echo;list ${MYWORKFOLDERUGEX}/host_data;echo'
alias clearhost_data='echo;list ${MYWORKFOLDERUGEX}/host_data;echo;rm -frdv ${MYWORKFOLDERUGEX}/host_data;echo;mkdir ${MYWORKFOLDERUGEX}/host_data;echo;list ${MYWORKFOLDERUGEX}/host_data;echo'
alias cleanhost_data='echo;list ${MYWORKFOLDERUGEX}/host_data;echo;rm -frdv ${MYWORKFOLDERUGEX}/host_data;echo;list ${MYWORKFOLDERUGEX};echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "makehost_data" 'Create the host_data folder '${MYWORKFOLDERUGEX}/host_data >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "clearhost_data" 'Remove and Remake the host_data folder '${MYWORKFOLDERUGEX}/host_data >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "cleanhost_data" 'Remove the host_data folder '${MYWORKFOLDERUGEX}/host_data >> ${tempENVHELPFILEalias}

alias makehost_interface_info='echo;mkdir ${MYWORKFOLDERUGEX}/host_interface_info;echo;list ${MYWORKFOLDERUGEX}/host_interface_info;echo'
alias clearhost_interface_info='echo;list ${MYWORKFOLDERUGEX}/host_interface_info;echo;rm -frdv ${MYWORKFOLDERUGEX}/host_interface_info;echo;mkdir ${MYWORKFOLDERUGEX}/host_interface_info;echo;list ${MYWORKFOLDERUGEX}/host_interface_info;echo'
alias cleanhost_interface_info='echo;list ${MYWORKFOLDERUGEX}/host_interface_info;echo;rm -frdv ${MYWORKFOLDERUGEX}/host_interface_info;echo;list ${MYWORKFOLDERUGEX};echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "makehost_interface_info" 'Create the host_interface_info folder '${MYWORKFOLDERUGEX}/host_interface_info >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "clearhost_interface_info" 'Remove and Remake the host_interface_info folder '${MYWORKFOLDERUGEX}/host_interface_info >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "cleanhost_interface_info" 'Remove the host_interface_info folder '${MYWORKFOLDERUGEX}/host_interface_info >> ${tempENVHELPFILEalias}

alias makeReference='echo;mkdir ${MYWORKFOLDERUGEX}/Reference;echo;list ${MYWORKFOLDERUGEX}/Reference;echo'
alias clearReference='echo;list ${MYWORKFOLDERUGEX}/Reference;echo;rm -frdv ${MYWORKFOLDERUGEX}/Reference;echo;mkdir ${MYWORKFOLDERUGEX}/Reference;echo;list ${MYWORKFOLDERUGEX}/Reference;echo'
alias cleanReference='echo;list ${MYWORKFOLDERUGEX}/Reference;echo;rm -frdv ${MYWORKFOLDERUGEX}/Reference;echo;list ${MYWORKFOLDERUGEX};echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "makeReference" 'Create the Reference folder '${MYWORKFOLDERUGEX}/Reference >> ${tempENVHELPFILEalias}
...
# Add functions to save the B4CP in a NOW dump Folder
#

printf "${tCYAN}%-35s${tNORM} : %s\n" "dumpB4CPLogs" 'Create dump now of the B4CPLogs folder '/var/log/tmp/b4CPscripts' in a '${MYWORKFOLDERUGEX}/dump' folder for today with DTG NOW' >> ${tempENVHELPFILEalias}

dumpB4CPLogs () 
{
    dumpnowfoldername=`DTGSDATE`'B4CP_logs'
    dumpnowfolder=${MYWORKFOLDERDUMP}/`date +%Y`/`date +%Y-%m`/${dumpnowfoldername}
    mkdir -pv "${dumpnowfolder}"
    mkdir -pv "${dumpnowfolder}/b4CPscripts"
    workfolder=/var/log/tmp/b4CPscripts
    cp -a -f -v ${workfolder}/ ${dumpnowfolder}/b4CPscripts/
    tar czvf ${dumpnowfolder}/b4CPscripts.logs.`DTGSDATE`.tgz --directory=${dumpnowfolder} ${workfolder}
    echo
    list -R "${dumpnowfolder}/"
    echo
}


alias showB4CPLogs='echo;list -R /var/log/tmp/b4CPscripts;echo'
alias makeB4CPLogs='echo;mkdir /var/log/tmp/b4CPscripts;echo;list /var/log/tmp/b4CPscripts;echo'
alias clearB4CPLogs='echo;list /var/log/tmp/b4CPscripts;echo;rm -frdv /var/log/tmp/b4CPscripts;echo;mkdir /var/log/tmp/b4CPscripts;echo;list /var/log/tmp/b4CPscripts;echo'
alias cleanB4CPLogs='echo;list /var/log/tmp/b4CPscripts;echo;rm -frdv /var/log/tmp/b4CPscripts;echo;list ${MYWORKFOLDERUGEX};echo'
printf "${tCYAN}%-35s${tNORM} : %s\n" "showB4CPLogs" 'Show contents of the B4CPLogs folder '/var/log/tmp/b4CPscripts >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "makeB4CPLogs" 'Create the B4CPLogs folder '/var/log/tmp/b4CPscripts >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "clearB4CPLogs" 'Remove and Remake the B4CPLogs folder '/var/log/tmp/b4CPscripts >> ${tempENVHELPFILEalias}
printf "${tCYAN}%-35s${tNORM} : %s\n" "cleanB4CPLogs" 'Remove the B4CPLogs folder '/var/log/tmp/b4CPscripts >> ${tempENVHELPFILEalias}
...
watch_S1C_connection () 
{
    echo 'S1C Tunnel: ...' ; echo
#    ccommand="clish -c 'show snapshots'"
    ccommand="echo 'S1C Tunnel: ...' ; echo ; clish -c 'show security-gateway cloud-mgmt-service' ; echo ; echo 'Policy Status:' ; echo ; fw stat ; echo ; echo 'Routes:' ; echo ; route -vn ; echo"
...
if [ -r ${MYWORKFOLDERSCRIPTSB4CP}/watch_accel_stats ] ; then
    ...
    alias show_S1C_connection='echo "S1C Tunnel:" ; echo ; clish -c "show security-gateway cloud-mgmt-service" ; echo ; echo "Policy Status:" ; echo ; fw stat ; echo ; echo "Routes:" ; echo ; route -vn'
    printf "${tCYAN}%-35s${tNORM} : %s\n" "show_S1C_connection" 'Gateway : Show Smart-1 Cloud MaaS Tunnel Status with policy and routes' >> ${tempENVHELPFILEalias}
    
    printf "${tCYAN}%-35s${tNORM} : %s\n" "watch_S1C_connection" 'Gateway : Watch Smart-1 Cloud MaaS Tunnel Status with policy and routes' >> ${tempENVHELPFILEalias}
...
alias generate_script_links='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/generate_script_links.v'${ScriptVersion}'.sh'
alias remove_script_links='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/remove_script_links.v'${ScriptVersion}'.sh'
alias reset_script_links='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/remove_script_links.v'${ScriptVersion}'.sh;echo;${MYWORKFOLDERUGEXSCRIPTS}/generate_script_links.v'${ScriptVersion}'.sh;echo;list'
...
alias make_local_snapshot_and_backup='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/Common/make_local_snapshot_and_backup.v'${ScriptVersion}'.sh'
printf "${tCYAN}%-35s${tNORM} : %s\n" "make_local_snapshot_and_backup" 'Make local Gaia Snapshot and Backup' >> ${tempENVHELPFILEalias}
alias make_snapshot_and_backup_to_ftp='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/Common/make_snapshot_and_backup_to_ftp_target.v'${ScriptVersion}'.sh'
...
# Updated 2024-06-11
if [ -r ${MYWORKFOLDERSCRIPTSB4CP}/Reset_SmartLog_Indexin ] ; then
...
alias alias_commands_add_user='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/UserConfig/alias_commands_add_user.v'${ScriptVersion}'.sh'
alias alias_commands_add_all_users='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/UserConfig/alias_commands_add_all_users.v'${ScriptVersion}'.sh'
...
alias alias_commands_update_user='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/UserConfig/alias_commands_update_user.v'${ScriptVersion}'.sh'
alias alias_commands_update_all_users='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/UserConfig/alias_commands_update_all_users.v'${ScriptVersion}'.sh'
...
alias reset_all_users_home_profile_files='cd ${MYWORKFOLDERUGEX};${MYWORKFOLDERUGEXSCRIPTS}/UserConfig/reset_all_users_home_profile_files.v'${ScriptVersion}'.sh'
...
alias byebye='echo ; historydumpnow ; echo ; history ; exit'

printf "${tCYAN}%-35s${tNORM} : %s\n" "byebye" 'Exit with Dump History now (dumphistorynow), and show history, before byexit' >> ${tempENVHELPFILEalias}
