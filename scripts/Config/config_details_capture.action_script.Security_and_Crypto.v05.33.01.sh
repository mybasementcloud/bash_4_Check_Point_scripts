...
#
# SCRIPT Template for bash scripts subscripts or action scripts - Config Caputure Details Action Script - Security_and_Crypto
#
#
SubScriptDate=2023-02-17
SubScriptVersion=05.33.01
SubScriptRevision=000
SubScriptSubRevision=150
TemplateVersion=05.33.01
TemplateLevel=006
SubScriptsLevel=010
SubScriptsVersion=05.33.01
SubScriptTemplateVersion=05.33.01
#
...
SubScriptFileNameRoot=config_details_capture.action_script.Security_and_Crypto
SubScriptShortName="config_details_capture.Security_and_Crypto.${SubScriptsLevel}"
SubScriptDescription="Template for bash scripts subscripts or action scripts - Config Caputure Details Action Script - Security_and_Crypto"
...
#----------------------------------------------------------------------------------------
# bash - collect openssl.conf and copy if it exists
#----------------------------------------------------------------------------------------

export command2folder=Security_and_Crypto
# /var/opt/CPshrd-R81.10/conf/openssl.cnf
export file2copy=openssl.cnf
export file2copypathfqfp="${CPDIR}/conf"

export outputfilenameaddon=
CopyFileAndDump2FQDNOutputfile

export file2find=openssl*.cnf*
...
#----------------------------------------------------------------------------------------
# clish ssl tls configuration
#----------------------------------------------------------------------------------------


export command2folder=configuration
export command2run=clish_configuration_ssl_tls
export outputfile=${outputfileprefix}'_'${command2run}${outputfilesuffix}${outputfiletype}
if [ -z ${command2folder} ] ; then
    export outputfilefqdn=${outputfilepath}
    export outputfilefqfn=${outputfilepath}${outputfile}
...
echo 'clish show configuration ssl tls :' >> ${outputfilefqfn}
echo >> ${outputfilefqfn}
echo '----------------------------------------------------------------------------' >> ${outputfilefqfn}
echo >> ${outputfilefqfn}
...
clish -c "show configuration ssl tls" >> ${outputfilefqfn}
echo >> ${outputfilefqfn}
