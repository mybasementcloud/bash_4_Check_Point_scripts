...
IdentifyGaiaVersionAndInstallationType () {
    ...
    if ${AuthenticationMaaS} ; then
        ...
        Check4SMSR81x10=`grep -c "Security Management Server R81.10 " ${workfile}`
        Check4SMSR81x20=`grep -c "Security Management Server R81.20 " ${workfile}`
        Check4SMSR82=`grep -c "Security Management Server R82 " ${workfile}`
        rm ${workfile}
        ...
        elif [ ${Check4SMSR81x10} -gt 0 ]; then
            echo `${dtzs}`${dtzsep} "Security Management Server version R81.10" >> ${gaiaversionoutputfile}
            #export gaiaversion=R81.10
            if [[ $(${CPDIR}/bin/cpprod_util UepmIsEps 2> /dev/null) == *"1"* ]]; then
                Check4EPM=1
                echo `${dtzs}`${dtzsep} "Endpoint Security Server version R81.10" >> ${gaiaversionoutputfile}
            else
                export Check4EPM=0
            fi
        elif [ ${Check4SMSR81x20} -gt 0 ]; then
            echo `${dtzs}`${dtzsep} "Security Management Server version R81.20" >> ${gaiaversionoutputfile}
            #export gaiaversion=R81.20
            if [[ $(${CPDIR}/bin/cpprod_util UepmIsEps 2> /dev/null) == *"1"* ]]; then
                Check4EPM=1
                echo `${dtzs}`${dtzsep} "Endpoint Security Server version R81.20" >> ${gaiaversionoutputfile}
            else
                export Check4EPM=0
            fi
        elif [ ${Check4SMSR82} -gt 0 ]; then
            echo `${dtzs}`${dtzsep} "Security Management Server version R82" >> ${gaiaversionoutputfile}
            #export gaiaversion=R82
            if [[ $(${CPDIR}/bin/cpprod_util UepmIsEps 2> /dev/null) == *"1"* ]]; then
                Check4EPM=1
                echo `${dtzs}`${dtzsep} "Endpoint Security Server version R82" >> ${gaiaversionoutputfile}
            else
                export Check4EPM=0
            fi
