...
ScriptAPIVersionCheck () {
    ...
    if [ $(expr ${MinAPIVersionRequired} '<=' ${CurrentAPIVersion}) -eq 1 ] ; then
...
subHandleMgmtCLIPublish () {
    ...
    if ${script_use_publish} ; then
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} 'Publish changes!' | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        mgmt_cli publish -s ${APICLIsessionfile} --conn-timeout ${APICLIconntimeout} -f json >> ${logfilepath} 2>> ${APICLIsessionerrorfile}
        EXITCODE=$?
...
# -------------------------------------------------------------------------------------------------
# subHandleMgmtCLIDiscard - discard changes if needed
# -------------------------------------------------------------------------------------------------
...
subHandleMgmtCLIDiscard () {
    #
    # ssubHandleMgmtCLIDiscard - discard changes if needed
    #
    ...
    #export APICLIsessionerrorfile=id.`date +%Y%m%d-%H%M%S%Z`.err
    echo `${dtzs}`${dtzsep} >> ${logfilepath}
    echo `${dtzs}`${dtzsep} 'mgmt_cli discard operation' >> ${logfilepath}
    echo `${dtzs}`${dtzsep} >> ${logfilepath}
    echo `${dtzs}`${dtzsep} >> ${APICLIsessionerrorfile}
    echo `${dtzs}`${dtzsep} 'mgmt_cli discard operation' >> ${APICLIsessionerrorfile}
    echo `${dtzs}`${dtzsep} >> ${APICLIsessionerrorfile}
    
    if ${script_use_publish} ; then
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} 'Discard changes!' | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        mgmt_cli discard -s ${APICLIsessionfile} --conn-timeout ${APICLIconntimeout} -f json >> ${logfilepath} 2>> ${APICLIsessionerrorfile}
        EXITCODE=$?
        cat ${APICLIsessionerrorfile} >> ${logfilepath}
        ...
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} 'Nothing to Discard!' | tee -a -i ${logfilepath}
        echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
        EXITCODE=0
    fi
...
subHandleMgmtCLILogout () {
    ...
    echo `${dtzs}`${dtzsep} | tee -a -i ${logfilepath}
    mgmt_cli logout -s ${APICLIsessionfile} -f json >> ${logfilepath} 2>> ${APICLIsessionerrorfile}
    EXITCODE=$?
    cat ${APICLIsessionerrorfile} >> ${logfilepath}
...
# DISCARD     - Execute API discard based on previous login and session file
