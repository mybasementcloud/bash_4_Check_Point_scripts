...
export BASHScriptName="migrate_export_npm_ugex.v$ScriptVersion"
    ...
    while [ -n "$1" ]; do
        ...
        while [ x"$OPT" != x"-" ] ; do
            case "$OPT" in
                ...
                --toolversion=* )
                    CLIparm_l01_toolvername="${OPT#*=}"
                    ;;
                --toolpath=* )
                    CLIparm_l02_toolpath="${OPT#*=}"
                    ;;
                # and --flag value opts like this
                --toolversion )
                    CLIparm_l01_toolvername="$2"
                    shift
                    ;;
                --toolpath )
                    CLIparm_l02_toolpath="$2"
                    shift
                    ;;
