...
mgmt_cli -r true -s $sessionfile show sessions limit 500 details-level full --format json | jq -r '.objects[] | select((.locks==0) and (.changes==0) and (."expired-session"==true)) | [ .uid ] | @csv' >> $deletefile
