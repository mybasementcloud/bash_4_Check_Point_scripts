...
#
# SCRIPT for BASH to generate a folder based on the current time and date
# run hcp script, and dump result files into the dtg folder
# Based on sk171436 - HealthCheck Point (HCP) Release Updates 
# Link: https://supportcenter.us.checkpoint.com/supportcenter/portal?eventSubmit_doGoviewsolutiondetails=&solutionid=sk171436
#
#
...
export BASHScriptFileNameRoot=install_hcp_offline
export BASHScriptShortName="hcp_install"
export BASHScriptnohupName=${BASHScriptShortName}
export BASHScriptDescription="Install hcp offline because not installed yet"
...
export BASHScripttftptargetfolder="healthchecks"
...
export hcpexecutablefilepath=

export hcpexecutablegasource=hcp

   18  cd ../download/HCP.T024.2021-01-26/
   19  list
   20  autoupdatercli install ./Check_Point_HCP_AUTOUPDATE_Bundle_T24_AutoUpdate.tar
   21  pwd
   22  autoupdatercli install `pwd`/Check_Point_HCP_AUTOUPDATE_Bundle_T24_AutoUpdate.tar
   23  /opt/CPInstLog/AutoUpdateLogs/hcp
   24  tail /opt/CPInstLog/AutoUpdateLogs/hcp
   25  gougex
   26  history
   27  hcp -r all
   28  hcp --show-last
   29  cp /var/log/hcp/last/hcp_last_report.tar.gz .
   30  godumpnow
   31  hcp -r all --include-wts yes --include-topology yes --include-charts yes -o `pwd`/hcp.`HOSTCPVERSIONNOW`.tgz
   32  list
   33  cat hcp.CORE-G2-Mgmt-1.R81.2021-02-19-160528CST.tgz
   34  hcp -r all --include-wts yes --include-topology yes --include-charts yes -o `pwd`/hcp.`HOSTCPVERSIONNOW`.txt
   35  cp /var/log/hcp/last/hcp_last_report.tar.gz
   36  cp /var/log/hcp/last/hcp_last_report.tar.gz .
 
JSON File hcp_offline_info.json

export HCPCheckFolder=/opt/HealthCheckPoint

export jsonFileDetailsfile=hcp_offline_info.json
export jsonFileDetailspath=${scriptspathroot}/Health_Check
export jsonFileDetailsHCPOfflineInstaller=${jsonFileDetailspath}/${jsonFileDetailsfile}

export HCPOfflineInstallerPathroot=${scriptspathroot}/Health_Check

export HCPOfflineInstallerFolder=`cat ${jsonFileDetailsHCPOfflineInstaller} | $JQ '."hcp_info"."foldername"' -r`
export HCPOfflineInstallerFile=`cat ${jsonFileDetailsHCPOfflineInstaller} | $JQ '."hcp_info"."installerfile"' -r`
export HCPOfflineInstallerDate=`cat ${jsonFileDetailsHCPOfflineInstaller} | $JQ '."hcp_info"."FileDate"' -r`
export HCPOfflineInstallerVersion=`cat ${jsonFileDetailsHCPOfflineInstaller} | $JQ '."hcp_info"."FileVersion"' -r`
export HCPOfflineInstallerMD5hash=`cat ${jsonFileDetailsHCPOfflineInstaller} | $JQ '."hcp_info"."md5hash"' -r`
export HCPOfflineInstallerSHA1hash=`cat ${jsonFileDetailsHCPOfflineInstaller} | $JQ '."hcp_info"."sha1hash"' -r`

export HCPOfflineInstallerpath=${HCPOfflineInstallerPathroot}/${HCPOfflineInstallerfolder}/${HCPOfflineInstallerFile}

if [ -r "${HCPCheckFolder}" ] ; then
    # HCP appears to be installed
    
    
else
    # HCP does not appear to be installed
    
fi

if [ -r "${HCPOfflineInstallerpath}" ] ; then
    
    echo 'Found HCP installation package here : '${HCPOfflineInstallerpath} | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    # Using . to run the script causes it to exit the complete operation and does not finiish this calling script
    #. ${scriptspathroot}/Health_Check/healthcheck
    export hcpexecutablefilepath=${scriptspathroot}/Health_Check/healthcheck.sh
    
else
    
    echo 'Missing critical HCP installation package file!  Looking for '${HCPOfflineInstallerpath}'  Exiting...' | tee -a -i ${logfilepath}
...
if [ -r ${hcpexecutablegasource} ] ; then
    # check if the version in the GA location is different from what we have after an update
    ${hcpexecutablegasource} -u | tee -a -i ${logfilepath}
    
    if [ x"${hcpexecutablefilepath}" == x"" ] ; then
        # Somehow we didn't have a healthcheck script in the scripts package???
        # So use the GA version location and the one that should be there
        export hcpexecutablefilepath=${hcpexecutablegasource}
    fi
    
    healthcheckgaversion=`${hcpexecutablegasource} -v | grep "Version"`
    hcpexecutablesversion=`${hcpexecutablefilepath} | grep "Version"`
    
    if [ ${healthcheckgaversion} != ${hcpexecutablesversion} ] ; then
        export hcpexecutablefilepath=${hcpexecutablegasource}
    else
        export hcpexecutablefilepath=${hcpexecutablefilepath}
    fi
fi

if [ x"${hcpexecutablefilepath}" == x"" ] ; then
    
    echo 'Missing critical healthcheck script!  Exiting...' | tee -a -i ${logfilepath}
...
echo | tee -a -i ${logfilepath}
echo 'Using healthcheck script at '${hcpexecutablefilepath}' ...' | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}

${hcpexecutablefilepath}
