...
export BASHScriptFileNameRoot=run_hcp_and_copy_files
export BASHScriptShortName="hcp_run"
export BASHScriptnohupName=${BASHScriptShortName}
export BASHScriptDescription="Run hcp and move to Date Time Group folder"
...
export hcplastrunreport=/var/log/hcp/last/hcp_last_report.tar.gz


if [ -r "${scriptspathroot}/Health_Check/healthcheck.sh" ] ; then
    
    echo 'Found healthcheck script here : '${scriptspathroot}/Health_Check/healthcheck.sh | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
...
elif [ -r "${scriptsbase}/healthcheck" ] ; then
    
    echo 'Found healthcheck script here : '${scriptsbase}/healthcheck | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    # Using . to run the script causes it to exit the complete operation and does not finiish this calling script
    #. ${scriptsbase}/healthcheck
    export hcpexecutablefilepath=${scriptsbase}/healthcheck
    
elif [ -r "${scriptsrepository}/healthcheck" ] ; then
    
    echo 'Found healthcheck script here : '${scriptsrepository}/healthcheck | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    # Using . to run the script causes it to exit the complete operation and does not finiish this calling script
    #. ${scriptsrepository}/healthcheck
    export hcpexecutablefilepath=${scriptsrepository}/healthcheck
    
elif [ -r "${hcpexecutablegasource}" ] ; then
    
    echo 'Found GA healthcheck script here : '${hcpexecutablegasource} | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
