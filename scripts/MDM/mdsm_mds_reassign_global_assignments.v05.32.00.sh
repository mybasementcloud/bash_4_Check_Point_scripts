...
#
# SCRIPT MDSM MDS - Re-assign global-assignments on MDS
#
#
...
export BASHScriptFileNameRoot=mdsm_mds_reassign_global_assignments
export BASHScriptShortName=mds_reassign_global_assignment
export BASHScriptnohupName=${BASHScriptShortName}
export BASHScriptDescription="MDSM MDS - Re-assign global-assignments on MDS"
...
if ${IsR8XVersion} ; then
    ...
    echo | tee -a -i ${logfilepath}
    echo 'Build batch file header...' | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    echo '"global-domains","dependent-domains"' > ${logfilefolder}/global-assignment.header.csv
    cat ${logfilefolder}/global-assignment.header.csv > ${logfilefolder}/global-assignment.csv
    
    echo | tee -a -i ${logfilepath}
    echo 'Query mgmt_cli for global-assignments and generate CSV dump...' | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    mgmt_cli -r true --port ${MGMTSSLPORT} show global-assignments details-level full -f json | $JQ '.objects[] | [ .["global-domain"]["name"], .["dependent-domain"]["name"] ] | @csv' -r > ${logfilefolder}/global-assignment.data.csv
    
    echo | tee -a -i ${logfilepath}
    echo 'Assemble the pieces...' | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    cat ${logfilefolder}/global-assignment.data.csv >> ${logfilefolder}/global-assignment.csv
    
    echo | tee -a -i ${logfilepath}
    cat ${logfilefolder}/global-assignment.csv | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    echo | tee -a -i ${logfilepath}
    echo 'Execute the batch of global-assignments...' | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    mgmt_cli -r true --port ${MGMTSSLPORT} -f json assign-global-assignment --batch ${logfilefolder}/global-assignment.csv | tee -a -i ${logfilepath}
    
else
    # Do something else because not R8X
    
    echo | tee -a -i ${logfilepath}
    echo 'NOT R8X...' | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
fi
