...
export BASHScriptFileNameRoot=backup_mds_w_logs_ugex
export BASHScriptShortName=backup_mds_w_logs_ugex
export BASHScriptnohupName=${BASHScriptShortName}
export BASHScriptDescription="Execute mds_backup to /var/log/__customer/upgrade_export/backups folder using mds_backup"
...
#----------------------------------------------------------------------------------------
# Document version of operation in the folder
#----------------------------------------------------------------------------------------

echo ${gaiaversion} > ${outputpathbase}/${gaiaversion}
echo >> ${outputpathbase}/${gaiaversion}
clish -c "show version all" >> ${outputpathbase}/${gaiaversion}
echo >> ${outputpathbase}/${gaiaversion}
...
#----------------------------------------------------------------------------------------
# Execute MDS Backup, with logs
#----------------------------------------------------------------------------------------

export command2folder=
#export command2run='mds_backup -b -i -s -d'
#export outputfilename=${outputfileprefix}'_mds_backup-bisd'${outputfilesuffix}${outputfiletype}
export command2run='mds_backup -b -s -d'
export outputfilename=${outputfileprefix}'_mds_backup-bsd'${outputfilesuffix}${outputfiletype}
export outputfilefqdn=${outputfilepath}

if [ -z ${command2folder} ] ; then
    export outputfilefqfn=${outputfilepath}$/{outputfilename}
else
    export outputfilefqfn=${outputfilepath}${command2folder}/${outputfilename}
...
echo ' with output to filename    : '${outputfile}name | tee -a -i ${logfilepath}
...
#    mds_backup -b -i -s -d "${outputfilepath}" | tee -a -i ${outputfilefqfn}
#else
#    # test mode
...
echo 'Execute > '${command2run}' '"${outputfilepath}" | tee -a -i ${logfilepath}


echo 'Execute > '${command2run}' '"${outputfilepath}" | tee -a -i ${outputfilefqfn}
echo | tee -a -i ${outputfilefqfn}
echo '--------------------------------------------------------------------------' | tee -a -i ${outputfilefqfn}
echo '--------------------------------------------------------------------------' | tee -a -i ${outputfilefqfn}
echo | tee -a -i ${outputfilefqfn}

mds_backup -b -s -d "${outputfilepath}" | tee -a -i ${outputfilefqfn}
