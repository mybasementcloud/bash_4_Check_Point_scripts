#!/bin/bash
#
# SCRIPT generate an array of domains and document their sessions details
#
...
export BASHScriptFileNameRoot=show_sessions_all_domains
export BASHScriptShortName=${BASHScriptFileNameRoot}
export BASHScriptnohupName=${BASHScriptShortName}
export BASHScriptDescription="Generate an array of domains and document their sessions details"
...
echo '------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}
echo 'Show list of each domains sessions' | tee -a -i ${logfilepath}
echo '------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
echo | tee -a -i ${logfilepath}
for j in "${DOMAINSARRAY[@]}"
do
    currentdomain=${j}
    domainnoquotes=${j//\"}
    domainnospaces="${domainnoquotes// /_}"
    
    echo '------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
    echo ' Domain :  '${currentdomain}'  :  '${domainnoquotes}'  :  '${domainnospaces} | tee -a -i ${logfilepath}
    echo '------------------------------------------------------------------------------' | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    sessionfile=${logfilepathbase}/session.${domainnospaces}.$DATEDTGS.txt
    if [ "${domainnoquotes}" = "${domainnospaces}" ] ; then
        mgmt_cli login -r true --port ${MGMTSSLPORT} -d "${domainnoquotes}" --format json > ${sessionfile}
    else
        mgmt_cli login -r true --port ${MGMTSSLPORT} -d "${currentdomain}" --format json > ${sessionfile}
    fi
    
    if ${B4CPSCRIPTVERBOSE} ; then
        echo 'Login Session Details:' | tee -a -i ${logfilepath}
        cat ${sessionfile} | tee -a -i ${logfilepath}
        echo | tee -a -i ${logfilepath}
    else
        echo 'Login Session Details:' >> ${logfilepath}
        cat ${sessionfile} >> ${logfilepath}
        echo >> ${logfilepath}
    fi
    
    #if [ "${domainnoquotes}" = "${domainnospaces}" ] ; then
        #mgmt_cli -s ${sessionfile} show sessions details-level full --format json | $JQ -r '.objects[] | ("\"'${domainnoquotes}'\" :  " + .uid + ", " + (.locks|tostring) + ", " + (.changes|tostring) + ", " + (."expired-session"|tostring) + ", " + ."user-name")' | tee -a -i ${logfilepath}
    #else
        #mgmt_cli -s ${sessionfile} show sessions details-level full --format json | $JQ -r '.objects[] | ("\"'"${currentdomain}"'\" :  " + .uid + ", " + (.locks|tostring) + ", " + (.changes|tostring) + ", " + (."expired-session"|tostring) + ", " + ."user-name")' | tee -a -i ${logfilepath}
    #fi
    
    jqquerystring='.objects[] | ("\"'
    if [ "${domainnoquotes}" = "${domainnospaces}" ] ; then
        jqquerystring=${jqquerystring}${domainnoquotes}
    else
        # The commented out varriants generate an error in JQ compilation
        #
        #jqquerystring=${jqquerystring}"${currentdomain}"
        #jqquerystring=${jqquerystring}\"${domainnoquotes}\"
        #jqquerystring=${jqquerystring}${currentdomain}
        jqquerystring=${jqquerystring}"${domainnoquotes}"
    fi
    jqquerystring="${jqquerystring}"'\" :  " + .uid + ", " + (.locks|tostring) + ", " + (.changes|tostring) + ", " + (."expired-session"|tostring) + ", " + ."user-name")'
    
    if ${B4CPSCRIPTVERBOSE} ; then
        echo 'JQ QueryString :  '"${jqquerystring}" | tee -a -i ${logfilepath}
        echo | tee -a -i ${logfilepath}
    else
        echo 'JQ QueryString :  '"${jqquerystring}" >> ${logfilepath}
        echo >> ${logfilepath}
    fi
    
    echo 'Domain Name :  UID, Number of Locks, Number of Changes, Expired Session, Session User Name' | tee -a -i ${logfilepath}
    echo | tee -a -i ${logfilepath}
    
    mgmt_cli -s ${sessionfile} show sessions details-level full --format json | $JQ -r "${jqquerystring}" | tee -a -i ${logfilepath}
    
    echo | tee -a -i ${logfilepath}
    
    if ${B4CPSCRIPTVERBOSE} ; then
        mgmt_cli logout -s ${sessionfile} | tee -a -i ${logfilepath}
    else
        mgmt_cli logout -s ${sessionfile} >> ${logfilepath}
    fi
